...
check_empty <- tibble(
  ...
  filter ( !str_detect (var_name,"Mkt")) # escludo xchè so già che è vuota ma potrebbe essere utile in futuro
...
+ Variabili categoriche (che sembrerebbero utili) ma sono *QUASI* sempre vuote:
  + `Natura Giuridica` e `Descrizione Natura Giuridica` sono quasi sempre vuote (99.7%)... è un errore rispetto `Forma Giuridica`?
  + `Sintesi Oggetto` e `Descrizione Sintesi Oggetto` sono quasi sempre vuote (99.5%)...  

+ Strano che `Categoria` è vuoto nel 20.7%, mentre `Descrizione Categoria` è vuoto solo nello 0.1%... ma le 2 variabili si parlano? (si, ci sono 363 sotto "Inesistente" x cui la Categoria è NA)
